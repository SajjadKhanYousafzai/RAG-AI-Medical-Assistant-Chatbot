...
def clean_response(text):
    """Clean HTML tags and format response for better display"""
    if not text:
        return text
    ...
    if request.method=="POST":
        ...
        if user_input:
            ...
            try:
                ...
                # Clean the response to remove HTML tags and improve formatting
                cleaned_result = clean_response(result)

                messages.append({"role" : "assistant" , "content" : cleaned_result})
                session["messages"] = messages
